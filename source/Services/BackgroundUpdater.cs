    ...
    public class BackgroundUpdater
    {
        ...
        public void Start()
        {
            ...
            Task.Run(async () =>
            {
                ...
                {
                    var msg = StringResources.GetString("LOCFriendsAchFeed_Error_Periodic_InitialCheckFailed", "Periodic update initial check failed.");
        ...
        private async Task PeriodicUpdateLoop(TimeSpan interval, CancellationToken token)
        {
            while (!token.IsCancellationRequested)
            {
                ...
                {
                    var msg = StringResources.GetString("LOCFriendsAchFeed_Error_Periodic_UpdateFailed", "Periodic update failed.");
                    _logger.Error(ex, msg);
                }
        ...
        {
            var lastStatus = _feedService.GetLastRebuildStatus() ?? StringResources.RebuildCompleted;
